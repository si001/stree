...
//func WrapCells(cells []termbox.Cell, width uint) []termbox.Cell {
//	str := CellsToString(cells)
//	wrapped := wordwrap.WrapString(str, width)
//	wrappedCells := []termbox.Cell{}
//	i := 0
//	for _, _rune := range wrapped {
//		if _rune == '\n' {
//			wrappedCells = append(wrappedCells, termbox.Cell{_rune, StyleClear})
//		} else {
//			wrappedCells = append(wrappedCells, termbox.Cell{_rune, cells[i].Style})
//		}
//		i++
//	}
//	return wrappedCells
...
//func BuildCellWithXArray(cells []termbox.Cell) []CellWithX {
//	cellWithXArray := make([]CellWithX, len(cells))
//	index := 0
//	for i, cell := range cells {
//		cellWithXArray[i] = CellWithX{X: index, termbox.Cell: cell}
//		index += rw.RuneWidth(cell.Ch)
//	}
